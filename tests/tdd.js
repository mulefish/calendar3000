...
function showCalendar(activeMonth, activeYear) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            if (i === 0 && j < firstDay) {
                rows[i].push("_")
                // Too early
            } else if (dateCounter > daysInMonth(activeYear ,activeMonth )) {
                // Too late
                rows[i].push("_")
                //break;
            } else {
    ...
    let d = new Date(activeYear, activeMonth, 1)
    console.log(d)
    return rows
}
...
function getCalendarTest() {
    ...
    for ( let month = 0; month < 12; month++ ) {
        ...
        let rows = showCalendar(month, 2019 )
        console.log( month + " ::: " + JSON.stringify( rows ))
